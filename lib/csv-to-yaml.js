...
const convertCSV = (file, outdir, permissions, propertiesCache) => { // eslint-disable-line
  ...
  const schemas = convertRows(lines, permissions, propertiesCache); // eslint-disable-line
...
// TODO: Talk this over with Rob
// Think there is the potential to rationalise what this is attempting to do
// i.e. ensure that all descriptions are referenced

// Removed deleteWhenDescriptionErrors completely
// Actually more dangerous as leads to definition files being randomly deleted
// Think it was useful when there we inconsistencies in how the data model was produced
...
//   .map(k =>   setSchemaFilename(k, outdir));
// const schemas = (outfiles).map(file => normalizeText(fs.readFileSync(file, 'utf8')));
